...
const userInfo = async (req, res) => {
    ...
    const [withdraw] = await connection.query('SELECT * FROM withdraw WHERE `phone` = ? AND status != 2', [rows[0].phone]);
    let totalWithdraw = 0;
    withdraw.forEach((data) => {
        totalWithdraw += parseFloat(data.money);
    });
    const [safe] = await connection.query('select sum(amount) as amount  from safe where phone = ? and status = 1',[rows[0].phone]);
...
const promotion = async (req, res) => {
    ...
    const [user] = await connection.query('SELECT `phone`, `code`,`invite`, `roses_f`, `roses_f1`, `roses_today`,`team_reg_number`, `team_deposit_amount`, `team_deposit_number`, `team_first_deposit` FROM users WHERE `token` = ? ', [auth]);
    ...
    const [deposit] = await connection.query('SELECT count(*) as depositCount FROM users u INNER join recharge r on u.phone = r.phone WHERE u.invite = ? and r.type ="upi_manual"',[userInfo.code]);
    console.log("deposit ",deposit);

    const [depositAmountData] = await connection.query('SELECT sum(r.money) as depositAmount FROM users u INNER join recharge r on u.phone = r.phone WHERE u.invite = ? and r.type ="upi_manual"',[userInfo.code]);
    console.log("depositAmount ",depositAmountData);

    const [firstDepositCountData] = await connection.query('SELECT  count(DISTINCT(u.phone)) as firstDepositCount FROM users u INNER join recharge r on u.phone = r.phone WHERE u.invite = ? and r.type ="upi_manual"',[userInfo.code]);
...
const infoUserBank = async (req, res) => {
    ...
    const [withdraw] = await connection.query('SELECT * FROM withdraw WHERE `phone` = ? AND status != 2', [userInfo.phone]);
    let totalWithdraw = 0;
    withdraw.forEach((data) => {
        totalWithdraw += parseFloat(data.money);
    });
...
const withdrawal3 = async (req, res) => {
    ...
    let password = req.body.password;
    let balance   = 0;
    if (!auth || !money || !password || money < 149) {
        return res.status(200).json({
            message: 'Failed',
            status: false,
            timeStamp: timeNow,
    ...
    const [user] = await connection.query('SELECT `phone`, `code`,`invite`, `money`,`level` FROM users WHERE `token` = ? AND password = ?', [auth, md5(password)]);
    ...
    const [recharge] = await connection.query('SELECT * FROM recharge WHERE phone = ? AND status = 1 And type = "upi_manual"', [userInfo.phone]);
    const [minutes_1] = await connection.query('SELECT * FROM minutes_1 WHERE phone = ?', [userInfo.phone]);
    const [t_withdrawal] = await connection.query('SELECT * FROM withdraw WHERE phone = ? AND status != 2', [userInfo.phone]);
    const [k3_wining] = await connection.query('select sum( k.get ) as total from result_k3 k where phone = ? and k.get > 0',[userInfo.phone]);
    console.log("k3_wining ",k3_wining);
    const [wining_5d] = await connection.query('select sum( k.get ) as total from result_5d k where phone = ? and k.get > 0',[userInfo.phone]);
    console.log("5d_wining ",wining_5d);
    const [wingo_wining] = await connection.query('select sum( m.get ) as total from minutes_1 m where phone = ? and m.get > 0',[userInfo.phone]);
    console.log("wingo_wining ",wingo_wining);
    const totalWining = parseFloat(k3_wining[0].total)+parseFloat(wining_5d[0].total)+parseFloat(wingo_wining[0].total);
...
const transfer = async (req, res) => {
    ...
    const [user] = await connection.query('SELECT `phone`,`money`, `code`,`invite`,`level` FROM users WHERE `token` = ? ', [auth]);
    let userInfo = user[0];
    let sender_phone = userInfo.phone;
    let sender_money = parseFloat(userInfo.money).toFixed(2);
    if (!user) {
        return res.status(200).json({
            message: 'Failed',
            status: false,
    ...
    try{
        ...
        if (parseFloat(sender_money) >= parseFloat(amount) && parseFloat(total2) >= parseFloat(amount) && parseFloat(total) >= parseFloat(amount) || userInfo.level == "2" && parseFloat(sender_money) >= parseFloat(amount)) {
            ...
            if (receiver.length === 1 && sender_phone !== receiver_phone) {
                let money = (parseFloat(sender_money) - parseFloat(amount)).toFixed(2);
                console.log('receiver money',receiver[0].total_money)
                let total_money =( parseFloat(amount) + parseFloat(receiver[0].total_money)).toFixed(2);
                console.log('total money ',total_money)
                await connection.query('UPDATE users SET money = ?,total_money=? WHERE phone = ?', [money,money,sender_phone]);
           
                await connection.query(`UPDATE users SET money = ?,total_money=? WHERE phone = ?`, [total_money,total_money,receiver_phone]);
            ...
            return res.status(200).json({
                message: `${sender_money < amount ? 'Your balance is not enough' : total < amount ? 'Your recharge is not enough to fulfill the request, Please recharge first.' :'The total bet is not enough to fulfill the request.'}`,
                status: false,
                timeStamp: timeNow,
            });
        }
...
const getInvitationData = async (req,res) => {
    try{
        ...
        if(user.length > 0){
        let userInfo = user[0];
        const [invitationList] = await connection.query("SELECT u.phone,u.code ,sum(IFNULL(r.money,0)) as money,r.type FROM users u left JOIN recharge r on u.phone = r.phone where (r.type='upi_manual' or r.type is Null) and (r.status=1 or r.status is Null) and u.invite= ? group by u.phone,u.code,r.type",[userInfo.code]);
...
try{
    ...
    if(user.length > 0){
        ...
        const total = totalBat[0].total ? totalBat[0].total : 0; 
        const [todayBat] = await connection.query('SELECT sum(money) as money FROM minutes_1 WHERE phone = ? and DATE_FORMAT(today, "%Y-%m-%d") = CURDATE()', [userInfo.phone]);
...
const transferRebate = async (req,res) => {
    ...
    try{
        ...
        const [user] = await connection1.query('SELECT `phone` FROM users WHERE `token` = ? ', [auth]);
        console.log("user ",user);
        if(user.length > 0){
            let userInfo = user[0];
            const [todayRebate] = await connection1.query('SELECT * FROM rebate WHERE phone = ? and DATE_FORMAT(time, "%Y-%m-%d") = CURDATE()', [userInfo.phone]);
            if(todayRebate.length == 0){
                ...
                await connection1.query(
                  ...
                  [rebateMoney, rebateMoney, userInfo.phone]
                );
                const [todayBet] = await connection.query('SELECT sum(money) as money FROM minutes_1 WHERE phone = ? and DATE_FORMAT(today, "%Y-%m-%d") = CURDATE()', [userInfo.phone]);
                await connection.query('Insert Into rebate set phone= ?, bets = ?, money = ?, time = ?',[userInfo.phone,todayBet[0].money,rebateMoney,timeNow])
...
const getRebateHistory = async (req,res) => {
    try{
        ...
        if(user.length > 0){
            let userInfo = user[0];
            const [rebateList] = await connection.query('SELECT * FROM rebate WHERE `phone` = ? ', [userInfo.phone]);
            res.status(200).json({
                message: `Rebats Data`,
                data:rebateList,
                status: true,
