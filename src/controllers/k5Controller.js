...
const betK5D = async (req, res) => {
    try {
        ...
        if (check >= 0) {
            ...
            try {
                const sql = `INSERT INTO result_5d SET id_product = ?, user_id = ?, phone = ?, code = ?, invite = ?, stage = ?, level = ?, money = ?, price = ?, amount = ?, fee = ?, game = ?, join_bet = ?, bet = ?, status = ?, time = ?`;
                await connection.execute(sql, [id_product, userInfo.id, userInfo.phone || '0', userInfo.code, userInfo.invite, period.period, userInfo.level, total, price, x, fee, game, join, list_join, 0, timeNow]);
            } catch (error) {
                ...
                await connection.execute(sql, [id_product, userInfo.phone || '0', userInfo.code, userInfo.invite, period.period, userInfo.level, total, price, x, fee, game, join, list_join, 0, timeNow]);
            }
            ...
            try {
                ...
                await connection.execute(sqlInsert, [id_product, userInfo.id, userInfo.phone || '0', userInfo.code, userInfo.invite, period.period, userInfo.level, price, x, fee, '5D', list_join, 0, checkTime, timeNow]);
            } catch (error) {
                ...
                await connection.execute(sqlInsert, [id_product, userInfo.phone || '0', userInfo.code, userInfo.invite, period.period, userInfo.level, price, x, fee, '5D', list_join, 0, checkTime, timeNow]);
            }
...
const GetMyEmerdList = async (req, res) => {
    ...
    const [result_5d] = await connection.query(
        `SELECT * FROM result_5d WHERE ${ownership.clause} AND game = ? ORDER BY id DESC LIMIT ${Number(pageno)},${Number(pageto)}`,
        [...ownership.params, String(game)]
    );
    const [result_5dAll] = await connection.query(
        `SELECT * FROM result_5d WHERE ${ownership.clause} AND game = ? ORDER BY id DESC`,
        [...ownership.params, String(game)]
    );
...
const handling5D = async(typeid) => {
    ...
    const [order] = await connection.execute(`SELECT id, user_id, phone, bet, price, money, fee, amount FROM result_5d WHERE status = 0 AND game = ${game} `);
    for (let i = 0; i < order.length; i++) {
        ...
        if (userId) {
            await connection.execute('UPDATE `users` SET `money` = `money` + ? WHERE `id` = ? ', [nhan_duoc, userId]);
        } else {
