  ...
  try {
    ...
    while (code) {
      ...
      const [deposit] = await connection.query(
        'SELECT count(*) as depositCount FROM users u INNER join recharge r on u.phone = r.phone WHERE u.invite = ? and r.type ="upi_manual" and r.status = 1',
        [code]
      );
      const [depositAmountData] = await connection.query(
        'SELECT IFNULL(sum(r.money),0) as depositAmount FROM users u INNER join recharge r on u.phone = r.phone WHERE u.invite = ? and r.type ="upi_manual" and r.status = 1',
        [code]
      );
      const [firstDepositCountData] = await connection.query(
        'SELECT  count(DISTINCT(u.phone)) as firstDepositCount FROM users u INNER join recharge r on u.phone = r.phone WHERE u.invite = ? and r.type ="upi_manual" and r.status = 1',
        [code]
      );
