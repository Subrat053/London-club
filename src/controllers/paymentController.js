...
const addManualUPIPaymentRequest = async (req, res) => {
    try {
        ...
        const normalizedUtr = rawUtr.replace(/\D/g, '');
        let utr = Number(normalizedUtr);
        const minimumMoneyAllowed = parseInt(process.env.MINIMUM_MONEY)

        if (!Number.isFinite(money) || !(money >= minimumMoneyAllowed)) {
        ...
        if (!normalizedUtr || normalizedUtr.length !== 12 || !Number.isFinite(utr)) {
            return res.status(200).json({
        ...
        const userIdentifier = user.phone || user.email || user.code || String(user.id);

        const pendingRechargeList = await rechargeTable.getRecordByPhoneAndStatus({ userId: user.id, phone: userIdentifier, status: PaymentStatusMap.PENDING, type: PaymentMethodsMap.UPI_GATEWAY })
...
const addManualUSDTPaymentRequest = async (req, res) => {
    try {
        const data = req.body
        let auth = req.cookies.auth;
        let money_usdt = Number(data?.money);
        ...
        if (!normalizedUtr || normalizedUtr.length !== 12 || !Number.isFinite(utr)) {
            return res.status(200).json({
                message: `Ref No. or UTR is Required And it should be 12 digit long`,
                status: false,
                timeStamp: timeNow,
            })
        }
...
const initiateUPIPayment = async (req, res) => {
    ...
    try {
        ...
        const pendingRechargeList = await rechargeTable.getRecordByPhoneAndStatus({ userId: user.id, phone: user.phone, status: PaymentStatusMap.PENDING, type: PaymentMethodsMap.UPI_GATEWAY })
...
const getUserDataByAuthToken = async (authToken) => {
    let [users] = await connection.query('SELECT `id`, `phone`, `email`, `code`,`name_user`,`invite` FROM users WHERE `token` = ? ', [authToken]);
    const user = users?.[0]
...
const addUserAccountBalance = async ({ money, userId, phone, invite }) => {
    const user_money = money + (money / 100) * 5
    const inviter_money = (money / 100) * 5

    if (userId) {
        await connection.query('UPDATE users SET money = money + ?, total_money = total_money + ? WHERE `id` = ?', [user_money, user_money, userId]);
    } else {
        await connection.query('UPDATE users SET money = money + ?, total_money = total_money + ? WHERE `phone` = ?', [user_money, user_money, phone]);
    }

    const [inviter] = await connection.query('SELECT id, phone FROM users WHERE `code` = ? LIMIT 1', [invite]);

    if (inviter.length) {
        console.log(inviter)
        console.log(inviter_money, inviter_money, invite, inviter?.[0].phone)
        if (inviter?.[0]?.id) {
            await connection.query('UPDATE users SET money = money + ?, total_money = total_money + ? WHERE `id` = ?', [inviter_money, inviter_money, inviter[0].id]);
        } else {
...
const rechargeTable = {
    getRecordByPhoneAndStatus: async ({ phone, userId, status, type }) => {
        ...
        const scopeClause = '(user_id = ? OR (phone = ? AND phone <> "" AND phone <> "0"))';
        const scopeParams = [userId || 0, phone || '__NO_PHONE__'];

        if (type) {
            [recharge] = await connection.query(`SELECT * FROM recharge WHERE ${scopeClause} AND status = ? AND type = ?`, [...scopeParams, status, type]);
        } else {
            [recharge] = await connection.query(`SELECT * FROM recharge WHERE ${scopeClause} AND status = ?`, [...scopeParams, status]);
        }
    ...
    create: async (newRecharge) => {
        ...
        try {
            await connection.query(
                `INSERT INTO recharge SET id_order = ?, transaction_id = ?, user_id = ?, phone = ?, money = ?, type = ?, status = ?, today = ?, url = ?, time = ?, utr = ?`,
                [newRecharge.orderId, newRecharge.transactionId, newRecharge.userId || 0, newRecharge.phone, newRecharge.money, newRecharge.type, newRecharge.status, newRecharge.today, newRecharge.url, newRecharge.time, newRecharge?.utr || "NULL"]
            );
        } catch (error) {
            await connection.query(
