...
const managerPortalPage = async(req, res) => {
    return res.render("daily/portal.ejs");
}
...
const canAccessManagedMember = (manager = {}, member = {}) => {
    const memberCtv = normalizeIdentity(member?.ctv);
    if (!memberCtv || memberCtv === '0') {
        return false;
    }
    return getCtvIdentityKeys(manager).includes(memberCtv);
}
...
const pageInfo = async(req, res) => {
    ...
    return res.render("daily/profileMember.ejs", {phone: identifier}); 
}
...
const userInfo = async(req, res) => {
    ...
    let { token, password, otp, level,...memberInfo } = member;

    if (!canAccessManagedMember(auths[0], memberInfo)) {
        return res.status(200).json({
            message: 'Failed',
            status: false, 
            timeStamp: timeNow,
    ...
    // cấp dưới trực tiếp all
    const [f1s] = await connection.query('SELECT `phone`, `code`,`invite`, `time` FROM users WHERE `invite` = ? ', [memberInfo.code]);
    ...
    const [bank_user] = await connection.query('SELECT * FROM user_bank WHERE phone = ? ', [member.phone || '__NO_PHONE__']);
    return res.status(200).json({
        message: 'Success',
        status: true,
        datas: memberInfo,
...
const listMember = async(req, res) => {
    ...
    const [users] = await connection.query(
        `SELECT id_user, email, phone, money, total_money, status, time FROM users WHERE veri = 1 AND level = 0 AND ${managerScope.clause} ORDER BY id DESC LIMIT ${pageno}, ${limit} `,
        managerScope.params
    );
    const [total_users] = await connection.query(`SELECT * FROM users WHERE veri = 1 AND level = 0 AND ${managerScope.clause} `, managerScope.params);
    return res.status(200).json({ 
        message: 'Success',
        status: true,
        datas: users,
