...
const betWinGo = async (req, res) => {
    ...
    console.log("type check sucessfull")
    ...
    const [user] = await connection.query('SELECT `id`, `phone`, `email`, `code`, `invite`, `level`, `money`,`vip_level` FROM users WHERE token = ? AND veri = 1  LIMIT 1 ', [auth]);
    console.log("wingo sucessfull",winGoNow[0])
    // console.log("user sucessfull",user[0])
    ...
    if (check >= 0) {
        ...
        // time = ?`;
                ...
                const sql = 'INSERT INTO minutes_1 (`id_product`,`user_id`,`phone`,`code`,`invite`,`stage`,`level`,`money`,`amount`,`fee`,`game`,`bet`,`status`,`today`,`time`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
                const per = [id_product, userInfo.id, userInfo.phone || '0', userInfo.code, userInfo.invite, period, userInfo.level, total, x, fee, gameJoin, join, 0, checkTime, timeNow];

                try {
                        await connection.execute(sql, per);
                } catch (error) {
                        const legacySql = 'INSERT INTO minutes_1 (`id_product`,`phone`,`code`,`invite`,`stage`,`level`,`money`,`amount`,`fee`,`game`,`bet`,`status`,`today`,`time`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
                        const legacyPer = [id_product, userInfo.phone || '0', userInfo.code, userInfo.invite, period, userInfo.level, total, x, fee, gameJoin, join, 0, checkTime, timeNow];
                        await connection.execute(legacySql, legacyPer);
                }
        ...
        //console.log("done by wing",userInfo);
        // const [level] = await connection.query('SELECT * FROM level ');
        // let level0 = level[0];
        // const sql2 = `INSERT INTO roses SET 
        // phone = ?,
...
const GetMyEmerdList = async (req, res) => {
    ...
    const [minutes_1] = await connection.query(
        `SELECT * FROM minutes_1 WHERE ${ownership.clause} AND game = ? ORDER BY id DESC LIMIT ${Number(pageno)},${Number(pageto)}`,
        [...ownership.params, game]
    );
    const [minutes_1All] = await connection.query(
        `SELECT * FROM minutes_1 WHERE ${ownership.clause} AND game = ? ORDER BY id DESC`,
        [...ownership.params, game]
    );
...
const addWinGo = async (game) => {
    try {
        ...
        console.log(`calling for  ${join}`,winGoNow);
        ...
        if (minPlayers.length >= 2) {
            ...
            console.log("enter in else if part");
            const betColumns = [
        ...
        if (nextResult == '-1') {
            await connection.execute(`UPDATE wingo SET amount = ?,status = ? WHERE period = ? AND game = "${join}"`, [amount, 1, period]);
             console.log("update wingo period where next result -1");
            newArr = '-1';
        } else {
            let result = '';
            ...
            console.log("update wingo period where next result not  -1");
        }
        ...
        const sql = "insert into `wingo`  (`period`,`amount`,`game`,`status`,`time`) values (?,?,?,?,?)";
        
        
        let per = [Number(period) + 1, 0, join, 0, timeNow];
        
        //console.log(sql,per);
         await connection.execute(sql, per, (err, results, fields) => {
          if (err) {
        ...
        //console.log("insert new",join,"at ",new Date().toString().replace(/T/, ':').replace(/\.\w*/, ''));
        console.log('insert new ',Number(period) + 1);
...
const handlingWinGo1P = async (typeid) => {
    console.log("calling handlingWinGo1P");
    let game = '';
    if (typeid == 1) game = 'wingo';
    if (typeid == 3) game = 'wingo3';
    if (typeid == 5) game = 'wingo5';
    ...
    console.log("minutes_1 updte with status 0 from  handlingWinGo1P");
    ...
    for (let i = 0; i < order.length; i++) {
        ...
        let users = [];
        if (userId) {
            [users] = await connection.execute('SELECT `money` FROM `users` WHERE `id` = ?', [userId]);
        }
        if (!users.length) {
            [users] = await connection.execute('SELECT `money` FROM `users` WHERE `phone` = ?', [phone]);
        }
        if (!users.length) {
            continue;
        }
        ...
        if (userId) {
            await connection.execute('UPDATE `users` SET `money` = ? WHERE `id` = ? ', [totals, userId]);
        } else {
            await connection.execute('UPDATE `users` SET `money` = ? WHERE `phone` = ? ', [totals, phone]);
        }
        console.log('handlingWinGo1P end')
    }
}
