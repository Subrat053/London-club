...
const transfer = async (req, res) => {
    ...
    // if (result == 0) {
        if (sender_money >= amount && total2 >= amount && total >= amount) {
            ...
            if (receiver.length === 1 && sender_phone !== receiver_phone) {
                ...
                let total_money = parseInt(amount) + parseInt(receiver[0].total_money);
            ...
            return res.status(200).json({
                message: `${sender_money < amount ? 'Your balance is not enough' :'The total bet is not enough to fulfill the request'}`,
                status: false,
                timeStamp: timeNow,
            });
        }
