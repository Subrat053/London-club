...
const betWinGo = async (req, res) => {
    ...
    const [user] = await connection.query('SELECT `phone`, `code`, `invite`, `level`, `money`,`vip_level` FROM users WHERE token = ? AND veri = 1  LIMIT 1 ', [auth]);
    ...
    if (check >= 0) {
        ...
        const sql = "insert into minutes_1  (`id_product`,`phone`,`code`,`invite`,`stage`,`level`,`money`,`amount`,`fee`,`game`,`bet`,`status`,`today`,`time`) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?) ";
        
        
        let per = [id_product, userInfo.phone, userInfo.code, userInfo.invite, period, userInfo.level, total, x, fee, gameJoin, join, 0, checkTime, timeNow];
        
        
        connection.execute(sql, per, (err, results, fields) => {
          if (err) {
