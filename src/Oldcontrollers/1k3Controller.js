...
const betK3 = async (req, res) => {
    try {
        ...
        if (check >= 0) {
            ...
            today = ?,
            time = ?`;
            await connection.execute(sqlInsert, [id_product, userInfo.phone, userInfo.code, userInfo.invite, period.period, userInfo.level, price, xvalue, fee, typeGame,listJoin, 0, checkTime, timeNow]);
