...
const betK5D = async (req, res) => {
    try {
        ...
        if (check >= 0) {
            ...
            today = ?,
            time = ?`;
            await connection.execute(sqlInsert, [id_product, userInfo.phone, userInfo.code, userInfo.invite, period.period, userInfo.level, price, x, fee, '5D', list_join, 0, checkTime, timeNow]);
